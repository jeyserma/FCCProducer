...
class FCCProducer:
    ...
    def __init__(self, process, campaign, detector, storagedir, args):
        ...
        self.condor_queue = args.condor_queue if hasattr(args, "condor_queue") else "workday"
        self.condor_priority = args.condor_priority if hasattr(args, "condor_priority") else ""
        self.args = args
    ...
    def parse_yfsww(self, fOut, rootFn, seed, nevents):
        ...
        for parm in parms:
            if parm=="mw":
                mw = str(self.args.mw/1000.).rjust(12)
                yfsww_card = yfsww_card.replace(f"<{parm}>", mw)
            if parm=="ecm":
                ecm = str(self.args.ecm).rjust(12)
                yfsww_card = yfsww_card.replace(f"<{parm}>", ecm)
            if parm=="ww":
                ww = str(self.args.ww/1000.).rjust(12)
